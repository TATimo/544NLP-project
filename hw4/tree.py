...
################################ Added Functions ################################
    ...
    def markovization(self):
        nodes = list(self.bottomup())
        for n in nodes:
            if len(n.children) > 1:
                n0 = n.children[0].label
                n1 = n.children[1].label
                nn0 = '[' + n1 + ']'
                nn1 = '[' + n0 + ']'
                if '*' in n0:
                    n.children[0].label += nn0
                if '*' in n1:
                    n.children[1].label += nn1

    def parentAnnotation(self):
        nodes = list(self.bottomup())
        for n in nodes:
            if len(n.children) > 1:
                for c in n.children:
    ...
    def restoreTag(self):
        nodes = list(self.bottomup())
        for n in nodes:
            if '[' in n.label: n.label = n.label.split('[')[0]
