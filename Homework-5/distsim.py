...
def cossim_sparse(v1,v2):
    ...
    for i in v2:
        downyv += v2[i]**2
    return upv/(math.sqrt(downxv)*math.sqrt(downyv))

    #the result is quite reasonable, since dog and cat are animals but univesrsity is something else
...
def show_nearest(word_2_vec, w_vec, exclude_w, sim_metric):
    ...
    for i in word_2_vec:
        if i not in exclude_w:
            sim_book[i] = sim_metric(word_2_vec[i],w_vec)
    for i in sorted(sim_book.items(), key=itemgetter(1),reverse=True)[:10]:
        n_words.append(i)
